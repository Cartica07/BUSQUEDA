...
// ---------- Login / logout ----------
// Cada código de error de Firebase Auth tiene un motivo distinto y conviene
// mostrarlo tal cual (traducido) en vez de un genérico "no se pudo", porque
// "contraseña incorrecta" y "ese correo no existe" o "se bloqueó por
// demasiados intentos" necesitan que la persona haga cosas distintas.
function traducirErrorLogin(err) {
  ...
  switch (codigo) {
    ...
    case 'auth/user-not-found':
      return 'No existe ninguna cuenta con ese correo en Firebase Authentication.';
    ...
    case 'auth/too-many-requests':
      return 'Se bloqueó temporalmente por demasiados intentos fallidos. Esperá un rato o restablecé la contraseña.';
    case 'auth/network-request-failed':
      return 'Falló la conexión a internet. Revisá tu red e intentá de nuevo.';
    case 'auth/operation-not-allowed':
      return 'El inicio de sesión con correo/contraseña está desactivado en el proyecto de Firebase (Authentication → Sign-in method).';
    default:
      return `No se pudo iniciar sesión${codigo ? ` (${codigo})` : ''}. Revisá el correo y la contraseña.`;
  }
}
...
function crearFilaAdmin(id, aviso) {
  ...
  const especieActual = aviso.especie === 'perro' || aviso.especie === 'gato' ? aviso.especie : '';
  ...
  row.innerHTML = `
    ...
    <div class="admin-row-body">
      ...
      <div class="admin-row-actions">
        ...
        <select class="admin-btn admin-select-especie" data-id="${id}" title="Clasificar especie (perro/gato)">
          <option value="" ${especieActual === '' ? 'selected' : ''}>Especie: sin clasificar</option>
          <option value="perro" ${especieActual === 'perro' ? 'selected' : ''}>Especie: Perro</option>
          <option value="gato" ${especieActual === 'gato' ? 'selected' : ''}>Especie: Gato</option>
        </select>
  ...
  row.querySelector('.admin-select-especie').addEventListener('change', (e) => {
    const valor = e.target.value;
    const ref = db.ref('avisos/' + id + '/especie');
    const accion = valor ? ref.set(valor) : ref.remove();
    accion.catch(err => alert('No se pudo actualizar la especie: ' + err.message));
  });
