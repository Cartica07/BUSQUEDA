...
// Modal elements (agregados en index.html)
...
let vectorFotoBuscada = null; // vector usado en la búsqueda rápida
let vectorFotoBuscadaFull = null; // vector de mayor resolución para refine
let modeloIA = null;
...
// Nuevo: crear thumbnail (dataURL) para usar en búsqueda rápida
async function makeThumbnailDataURL(file, maxSide = 400, quality = 0.7) {
  try {
...
// Las fotos de celular suelen venir en resolución enorme; achicarlas antes de analizarlas evita trabones.
// Se usa "contain" (relleno gris) para no recortar partes importantes.
...
// Deja respirar al navegador entre imágenes para no bloquear la UI en móviles débiles
function respirar() {
...
// Nuevo flujo: al cambiar el input abrimos el modal para elegir sección (no preguntamos precisión)
...
// Cancelar barra de búsqueda por foto
fotoBusquedaCancelar.addEventListener('click', () => {
  salirDeBusquedaPorFoto();
});

// Botón refinar (opcional): si existe lo conectamos
...
function openPhotoSectionModal() {
  if (!photoSearchModal) {
    // compatibilidad: si no hay modal, asumimos 'ambos'
...
// Al aceptar, arrancamos la búsqueda rápida (thumbnail) limitada a la sección elegida
...
// iniciarBusquedaPorFoto ahora usa thumbnail por defecto y no pregunta precisión
async function iniciarBusquedaPorFoto(file, opts = { section: 'ambos' }) {
  ...
  try {
    ...
    const chico = redimensionarImagen(img, 224); // reducido para rapidez
    const tensor = modeloIA.infer(chico, true);
...
async function renderPorFoto() {
  ...
  fotoBusquedaTexto.textContent = 'Mostrando resultados (rápido). Podés refinar para mayor precisión.';
...
async function refinarBusquedaPorFoto() {
  ...
  try {
    ...
    // Recalcular vector de la foto de usuario con mayor resolución
    if (!vectorFotoBuscadaFull) {
      const imgUser = await cargarImagen(fotoBusquedaPreview.src);
      const chicoFull = redimensionarImagen(imgUser, 640);
    ...
    for (let i = 0; i < candidatosParaRefinar.length; i++) {
      ...
      try {
        ...
        const canvas = redimensionarImagen(img, 480);
    ...
    const final = refinados.slice(0, 30);
